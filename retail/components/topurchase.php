              ...
              <tbody>
                <?php $c=1;
                    ...
                    while($res = mysqli_fetch_array($result)){
                      ...
                      if($results = mysqli_query($mysqli, "SELECT SUM(quantity) as quantity FROM sales,solditems WHERE  solditems.salesid=sales.id and ( sales.date BETWEEN '$fromdate' AND '$todate') and productid=$pid"))
                      while($ress = mysqli_fetch_array($results)){
                        $currsqty=$ress['quantity'];
                      }

                      $currpqty=$tpqty-$tsqty+1;
                      $expbuyqty=$qty*$tpqty/$tsqty;
                      $demand=ceil(abs($expbuyqty-($currpqty+$currsqty)));

                      if ($res['subcategory']=='DRY VEGETABLE' or $res['category']=='DAHI') {
                        $tobuyqty=round($demand/$daysRemaining)*7;
                      } else if ($res['category']=='EGG') {
                        $tobuyqty=round($demand/$daysRemaining)*5;
                      } else if ($res['category']=='GROCERY') {
                        $tobuyqty=round($demand/$daysRemaining)*20;
                      } else if ($res['category']=='VEGETABLE' or $res['category']=='MILKY') {
                        $tobuyqty=ceil($demand/$daysRemaining)*3;
                      } else {
                        $tobuyqty=ceil(($demand/$daysRemaining)*30);
                      }
